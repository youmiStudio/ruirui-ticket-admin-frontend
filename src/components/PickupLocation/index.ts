...
import type { BoxOptions, Action, IBox, BoxShortcutMethod } from './types';
...
const showBox = (options: any, appContext?: AppContext) => {
  ...
  options.onAction = (data: any) => {
    const currentBox = CustomBoxInstance.get(vm);
    let resolve: Action;
    if (typeof data === 'string') {
      resolve = data as Action;
    } else {
      resolve = data.action;
    }


    if (resolve === 'cancel' || resolve === 'close') {
      currentBox?.reject();
    } else {
      currentBox?.resolve(data);
    }
  };
